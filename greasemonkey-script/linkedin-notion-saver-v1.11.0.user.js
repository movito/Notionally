...
(function() {
    ...
    */
    ...
    async function init() {
        ...
        // For Pulse articles, also look for the dropdown button immediately
        if (isPulseArticle()) {
            log('🔍 PULSE ARTICLE DETECTED! URL:', window.location.pathname);
            log('Starting Pulse article button search...');
            
            let retryCount = 0;
            const maxRetries = 10;
            
            const checkForPulseButton = () => {
                retryCount++;
                log(`Attempt ${retryCount}/${maxRetries}: Searching for Pulse overflow button...`);
                
                // Try multiple selectors
                const selectors = [
                    '.reader-actions button[aria-label*="More"]',
                    '.reader-actions button[aria-label*="options"]',
                    '.reader-actions button[aria-label*="Open"]',
                    '.reader-actions button.artdeco-dropdown__trigger',
                    'button[aria-label*="Open options"]',
                    'button.reader-actions__overflow-button',
                    '.reader-actions__overflow button'
                ];
                
                let overflowButton = null;
                for (const selector of selectors) {
                    overflowButton = document.querySelector(selector);
                    if (overflowButton) {
                ...
                // Also log what we DO find in reader-actions
                const readerActions = document.querySelector('.reader-actions');
                if (readerActions) {
                    const buttons = readerActions.querySelectorAll('button');
                    log(`Found ${buttons.length} buttons in .reader-actions`);
                    buttons.forEach((btn, idx) => {
                        log(`  Button ${idx}: aria-label="${btn.getAttribute('aria-label')}", class="${btn.className}"`);
                    });
                }
                
                if (overflowButton) {
                    log('✅ FOUND Pulse overflow button! Adding click listener...');
                    
                    // Add both click and mousedown listeners
                    const attachMenuHandler = () => {
                        log('Pulse button clicked/activated, waiting for dropdown...');
                        
                        // Try multiple times to catch the dropdown
                        let checkCount = 0;
                        const checkForDropdown = setInterval(() => {
                            checkCount++;
                            // Look for any artdeco-dropdown__content that just appeared
                            const dropdowns = document.querySelectorAll('.artdeco-dropdown__content');
                            let targetDropdown = null;
                            
                            // Find the dropdown that has menu items (not all dropdowns are menus)
                            for (const dropdown of dropdowns) {
                                if (dropdown.querySelector('li, [role="menuitem"], .artdeco-dropdown__item')) {
                            ...
                            if (targetDropdown) {
                                log(`✅ Dropdown found after ${checkCount} checks`);
                                clearInterval(checkForDropdown);
                                
                                if (!targetDropdown.querySelector('.notionally-pulse-menu-item')) {
                                    log('Adding Save to Notion to Pulse dropdown');
                                    addSaveToPulseDropdown(targetDropdown);
                                } else {
                                    log('Save to Notion already in dropdown');
                                }
                            } else if (checkCount > 20) {
                                log('❌ Dropdown not found after 20 checks');
                                clearInterval(checkForDropdown);
                            }
                        }, 50);
                    };
                    
                    overflowButton.addEventListener('click', attachMenuHandler);
                    overflowButton.addEventListener('mousedown', attachMenuHandler);
                    
                    log('Event listeners attached to Pulse button');
                } else if (retryCount < maxRetries) {
                    // Retry if button not found yet
                    log(`Button not found, retrying in 1 second...`);
                    setTimeout(checkForPulseButton, 1000);
                } else {
                    log('❌ Failed to find Pulse button after maximum retries');
                }
            };
            
            // Start checking immediately and after a delay
            checkForPulseButton();
            setTimeout(checkForPulseButton, 2000); // Extra check after 2s for late-loading content
        }
        ...
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                ...
                // Check for PULSE ARTICLE dropdown content being populated
                // Look for artdeco-dropdown__content that appears on Pulse pages
                if (isPulseArticle() && mutation.target.classList && 
                    mutation.target.classList.contains('artdeco-dropdown__content')) {
                    if (mutation.addedNodes.length > 0) {
                        log('Pulse article dropdown content detected (artdeco-dropdown__content)');
                        const dropdownContent = mutation.target;
                        // Wait for content to be fully populated
                        setTimeout(() => {
                            // Check if this dropdown has menu items (not all dropdowns are menus)
                            const hasMenuItems = dropdownContent.querySelector('li, [role="menuitem"], .artdeco-dropdown__item');
                ...
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1) {
                        ...
                        // Check if it's a PULSE ARTICLE dropdown content div
                        if (isPulseArticle() && node.classList && 
                            node.classList.contains('artdeco-dropdown__content')) {
                            log('Found new Pulse article dropdown content node');
                            // Wait for content to populate
                            setTimeout(() => {
                                const hasMenuItems = node.querySelector('li, [role="menuitem"], .artdeco-dropdown__item');
                                if (hasMenuItems && !node.querySelector('.notionally-pulse-menu-item')) {
                                    log('Adding Save to Notion to newly found Pulse dropdown');
                                    addSaveToPulseDropdown(node);
                                }
                            }, 100);
                        }
    ...
    // Special handling for Pulse articles - ensure observer is active
    if (isPulseArticle()) {
        log('Pulse article detected, ensuring observer is active');
        // Re-check initialization after a delay to catch late-loading content
        setTimeout(() => {
            if (!document.body.classList.contains('notionally-initialized')) {
                log('Re-initializing for Pulse article');
                init();
            }
        }, 2000);
    }
