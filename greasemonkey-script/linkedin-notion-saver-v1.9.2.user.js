...
(function() {
    ...
    console.log('[notionally v1.9.2] Script loaded on:', window.location.href);
    ...
    function extractPulseArticleData() {
        ...
        // Extract author information - check multiple possible locations
        const authorSection = document.querySelector('.author-info-container, .article-author-info, [data-test-id*="author"]');
        if (authorSection) {
            const authorName = authorSection.querySelector('.text-body-medium, .author-name, h3');
            const authorHeadline = authorSection.querySelector('.text-body-small, .author-headline, .t-black--light');
        ...
        const articleContainer = document.querySelector('.article-content, .reader-article-content, [data-test-id*="article-content"], main article');
        if (articleContainer) {
    ...
    // ============ MAIN INITIALIZATION ============
    async function init() {
        log('Initializing notionally v1.9.2...');
        ...
        log('notionally v1.9.2 initialized successfully');
