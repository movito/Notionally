...
// @version      1.16.2
// @description  v1.16.2 - Fixed italic formatting and added visual feedback for Pulse saves
 ...
 * Version 1.16.2 - Fixed italic text preservation and added toast notifications
 * - Preserves italic/em tags in Pulse article content
 * - Added visual feedback (toast) when saving Pulse articles
 * Version 1.16.1 - Enhanced author extraction for Pulse articles
 * - Extracts author name from h2.text-heading-medium in reader-author-info
 * - Captures author profile URL from href attribute
 * - Includes author headline/subtitle
 * Version 1.16.0 - Extract and save Pulse article cover images
 * - Captures cover image from figure.reader-cover-image
 * - Sends to server for Dropbox storage and Notion embedding
 * Version 1.15.1 - Reduced maximum newlines from 3 to 2 for cleaner formatting
 * Version 1.15.0 - Custom text extraction for Pulse articles
 * - Added extractPulseFormattedContent function specifically for Pulse articles
 * - Eliminates excessive newlines while preserving article structure
 * - Feed post extraction remains unchanged (uses direct textContent - works perfectly)
 * - Separate extraction methods for feed posts vs Pulse articles (intentional design)
 * Version 1.13.0 - Combines working v1.7.5 feed posts with v1.9.5 Pulse articles
    ...
    */
    ...
    function extractPostData(postElement, postUrl = null) {
        try {
            ...
            // Extract post text - using direct textContent extraction (works perfectly for feed posts)
            // NOTE: Feed posts use different extraction than Pulse articles - this is intentional
            const textSelectors = [
    ...
    // ============ PULSE ARTICLE FUNCTIONS ============
    
    // Extract Pulse article data
    // Custom text extraction function specifically for Pulse articles
    // Enhanced from v1.9.5 to eliminate excessive newlines while preserving formatting
    function extractPulseFormattedContent(container) {
        let formattedText = '';
        ...
        const processNode = (node) => {
            ...
            if (node.nodeType === 1) {
                ...
                // Process paragraphs - recursively process children to preserve inline formatting
                else if (tagName === 'p') {
                    const children = Array.from(node.childNodes);
                    const paragraphContent = children.map(child => processNode(child)).join('');
                    if (paragraphContent.trim()) {
                        content = `\n\n${paragraphContent.trim()}\n\n`;
                    }
                }
                // Process lists - recursively process list items to preserve inline formatting
                else if (tagName === 'ul' || tagName === 'ol') {
                    const listItems = node.querySelectorAll('li');
                    if (listItems.length > 0) {
                        content = '\n\n';
                        listItems.forEach((li, index) => {
                            // Process list item children to preserve formatting
                            const children = Array.from(li.childNodes);
                            const itemContent = children.map(child => processNode(child)).join('').trim();
                            if (itemContent) {
                                const bullet = tagName === 'ol' ? `${index + 1}.` : '•';
                                content += `${bullet} ${itemContent}\n`;
                            }
                        });
                        content += '\n';
                    }
                }
                // Process blockquotes - recursively process to preserve inline formatting
                else if (tagName === 'blockquote') {
                    const children = Array.from(node.childNodes);
                    const quoteContent = children.map(child => processNode(child)).join('').trim();
                    if (quoteContent) {
    ...
    function extractPulseArticleData() {
        ...
        // Get title
        const title = document.querySelector('h1, .reader-article-header__title, [data-test-id*="title"]');
        articleData.title = title?.textContent.trim() || 'LinkedIn Pulse Article';
        
        // Get author information from reader-author-info section
        const authorSection = document.querySelector('.reader-author-info__content, .reader-author-info');
        if (authorSection) {
            // Get author name from h2 heading
            const authorNameElement = authorSection.querySelector('h2.text-heading-medium, h2');
            if (authorNameElement) {
                // Clean the text to remove extra whitespace and any badge text
                articleData.author = authorNameElement.textContent.trim().replace(/\s+/g, ' ');
                log('Found author name:', articleData.author);
            }
            
            // Get author profile URL from the link
            const authorLinkElement = authorSection.querySelector('a[href*="/in/"], a[href*="/company/"]');
            if (authorLinkElement) {
                const href = authorLinkElement.getAttribute('href');
                if (href) {
                    // Convert relative URL to absolute
                    if (href.startsWith('/')) {
                        articleData.authorProfileUrl = `https://www.linkedin.com${href}`;
                    } else if (!href.startsWith('http')) {
                        articleData.authorProfileUrl = `https://www.linkedin.com/${href}`;
                    } else {
                        articleData.authorProfileUrl = href;
                    }
                    log('Found author profile URL:', articleData.authorProfileUrl);
                }
            }
            
            // Get author headline/subtitle
            const authorHeadlineElement = authorSection.querySelector('.artdeco-entity-lockup__subtitle, .lt-line-clamp');
            if (authorHeadlineElement) {
                articleData.authorHeadline = authorHeadlineElement.textContent.trim();
                log('Found author headline:', articleData.authorHeadline);
            }
        }
        ...
        // Get article content with improved formatting (no excessive newlines)
        const articleContent = document.querySelector('.reader-article-content, article, [data-test-id*="article-content"]');
        if (articleContent) {
            // Use our custom Pulse article extraction function to avoid excessive newlines
            articleData.text = extractPulseFormattedContent(articleContent);
            articleData.content = articleData.text;
        } else {
            // Fallback to title if no content found
            articleData.text = articleData.title;
            articleData.content = articleData.title;
        }
        
        // Extract cover image from figure element
        const coverImage = document.querySelector('figure img.reader-cover-image__img, figure .reader-cover-image__wrapper-right-rail-layout img');
        if (coverImage) {
            const coverImageUrl = coverImage.src || coverImage.dataset.src;
            const coverImageCaption = document.querySelector('figure figcaption.reader-cover-image__caption')?.textContent.trim() || 
            ...
            log('Found cover image:', coverImageUrl);
        }
        
        // Get content images (excluding cover image)
        const contentImages = Array.from(document.querySelectorAll('.reader-article-content img, article img'))
            .filter(img => !img.classList.contains('reader-cover-image__img')) // Exclude cover image
            .map(img => ({
        ...
        // Combine cover image and content images for media field
        articleData.media = {
            images: []
        };
        ...
        if (articleData.coverImage) {
            articleData.media.images.push(articleData.coverImage);
        }
        
        // Add content images
        if (contentImages.length > 0) {
            articleData.media.images.push(...contentImages);
        }
        
        // Keep backward compatibility with images field
        articleData.images = articleData.media.images.map(img => img.url);
        ...
        saveMenuItem.addEventListener('click', async function(e) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    showToast('✅ Article successfully saved to Notion!', 'success');
                    setTimeout(() => {
    ...
    // Observe Pulse article dropdown
    ...
    async function init() {
        ...
        // Initialize Pulse article observer if on Pulse page
        if (isPulseArticle()) {
            log('📰 Pulse article detected');
            observePulseArticleDropdown();
        }
        
        // Check for navigation to Pulse articles (LinkedIn is SPA)
