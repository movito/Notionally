...
(function() {
    ...
    console.log('[Notionally v1.10.0] Script loaded on:', window.location.href);
    ...
    */
    ...
    async function init() {
        ...
        if (isPulseArticle()) {
            log('On Pulse article page, looking for overflow menu button');
            // Look for the three-dots button in the reader actions area
            const checkForPulseButton = () => {
                const overflowButton = document.querySelector('.reader-actions button[aria-label*="More"], .reader-actions button[aria-label*="options"]');
                if (overflowButton) {
                    log('Found Pulse overflow button, monitoring for clicks');
                    overflowButton.addEventListener('click', () => {
                        log('Pulse overflow button clicked, waiting for dropdown');
                        setTimeout(() => {
                            const dropdown = document.querySelector('.reader-overflow-options__content');
                            if (dropdown && !dropdown.querySelector('.notionally-pulse-menu-item')) {
                                log('Adding Save to Notion to Pulse dropdown after button click');
                                addSaveToPulseDropdown(dropdown);
                            }
                        }, 100);
                    });
                } else {
        ...
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                ...
                if (mutation.target.classList && mutation.target.classList.contains('reader-overflow-options__content')) {
                    if (mutation.addedNodes.length > 0) {
                        log('Pulse article dropdown content detected');
                ...
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1) {
                        ...
                        if (node.classList && node.classList.contains('reader-overflow-options__content')) {
